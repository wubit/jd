...
MAX_PER_CATEGORY = 100         # 每个分类最多爬取视频数 (0=不限制)
...
async def scrape_category(session, cat_id, cat_name, total_pages):
    """爬取单个分类的所有视频"""
    # 如果设置了每分类上限，只爬取需要的列表页数
    if MAX_PER_CATEGORY > 0:
        pages_needed = math.ceil(MAX_PER_CATEGORY / 40)  # 每页约40条
    ...
    # 限制播放页数量
    if MAX_PER_CATEGORY > 0:
        vodplay_urls = vodplay_urls[:MAX_PER_CATEGORY]
